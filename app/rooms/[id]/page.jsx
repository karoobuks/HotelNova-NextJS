...
//             <div>
//               <span className="font-medium text-gray-700">Availability:</span>{" "}
//               <span className={plainRoom.availability ? "text-green-700" : "text-red-600"}>
//                 {plainRoom.bookingStatus}
//               </span>
//             </div>
...
//               <div className="grid md:grid-cols-2 gap-6">
//                 {room.reviews.map((review, index) => (
//                   <ReviewCard key={index} review={review} currentUserId={currentUserId} />
//                 ))}
//               </div>
//             </div>
//           )}
...
// This file should be in `app/rooms/[id]/page.jsx` or similar
...
//         <div className="container mx-auto px-4 sm:px-6 lg:px-8 pt-6">
//           <Link href="/rooms" className="text-blue-600 text-center hover:underline flex items-center">
//             <span className="mr-2">←</span> Go Back to Rooms
//           </Link>
//         </div>
//       </section>
...
//                   const isOwner = currentUserId?.toString() === reviewerId;
...
const RoomPage = async ({ params }) => {
  ...
  // ✅ Plain object to avoid Next.js serialization issues
  const plainRoom = JSON.parse(JSON.stringify(room));
  ...
  if (currentBooking) {
    ...
    plainRoom.availability = true;
    plainRoom.bookingStatus = nextBooking
      ? "Open (future booking exists)"
      : "Open";
  }

  // Extract username from email if name is missing
  plainRoom.reviews.forEach((review) => {
    const user = review.reviewer;
  ...
  return (
    <>
      <RoomHeaderImage image={headerImage} />
      ...
      <section className="bg-gray-50">
        <div className="container mx-auto py-12 px-4 sm:px-6 lg:px-8">
          <h1 className="text-4xl font-bold mb-4 text-gray-800">{plainRoom.name}</h1>
          <p className="text-gray-700 mb-6 text-lg leading-relaxed">
            {plainRoom.description}
          </p>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-2">
              <span className="font-medium text-gray-700">Rating:</span>
              <StarRating rating={plainRoom.averageRating} />
            </div>

            <div className="mt-8">
              <h3 className="text-xl font-semibold mb-4">Amenities</h3>
              <div className="flex flex-wrap gap-2">
                {plainRoom.amenities.map((amenity, index) => (
                  <span
                    key={index}
                    className="text-sm font-semibold px-3 py-1.5 rounded-full bg-blue-100 text-blue-600"
                  >
                    {amenity}
                  </span>
                ))}
          ...
          {plainRoom.reviews?.length > 0 && (
            <div className="mt-14">
              <h2 className="text-2xl font-bold mb-6">Guest Reviews</h2>
              <div className="grid md:grid-cols-2 gap-6">
                {plainRoom.reviews.map((review) => (
                  <ReviewCard
                    key={review._id}
                    review={review}
                    isOwner={currentUserId === review.reviewer?._id?.toString()}
                  />
                ))}
              </div>
            </div>
      ...
      <RoomImages images={plainRoom.images} />
    </>
  );
};
